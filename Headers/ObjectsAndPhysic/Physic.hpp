//
// Created by Aspadien on 22.12.2025.
//

#ifndef CUBEREBUILD_PHYSIC_HPP
#define CUBEREBUILD_PHYSIC_HPP
...
class Physic {
    ...
    void makeTick(KeyboardControl &keyboard_control,std::vector<std::shared_ptr<Chunk>> &batchG, Camera &camera, const double dt,ChunkMap &chunkMap, const Window &window) {
        ...
        if (onGround) {
            kineticVector -= kineticVector * 0.91f * (dt/(dt+0.005f));
        }else {
...
if (const auto temp = MouseInteractions::castRayAndModifyBlockCached(window.window,camera,chunkMap,1,10); temp != nullptr) {
...
void movePlayer(AABB& playerAABB, glm::vec3& velocity, const double dt, bool& onGround, ChunkMap &chunks) {
