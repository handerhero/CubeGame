...
#include "../../Headers/ChunkSystem/ChunkGenerationManager.h"

#include <algorithm>

#include "../../Headers/Configuration/Config.h"
#include "../../Headers/Configuration/Constants.hpp"
...
void chunkWorker() {
    while (running) {
        ...
        // =================================================================================
        // НОВЫЙ БЛОК: Проверка актуальности задачи
        // =================================================================================

        // 2. Читаем ТЕКУЩУЮ позицию игрока (под мьютексом!)
        glm::ivec3 currentPlPos;
        {
            // Используем тот же мьютекс, что и в главном потоке при обновлении позиции
            std::lock_guard<std::mutex> lock(playerPosMutex);
            currentPlPos = currentPlayerChunk;
        }

        // 3. Считаем реальное расстояние сейчас
        // (Игрок мог телепортироваться или быстро улететь, пока задача лежала в очереди)
        auto dx = static_cast<float>(task.chunkPos.x - currentPlPos.x);
        auto dz = static_cast<float>(task.chunkPos.z - currentPlPos.z);

        // Квадрат расстояния (чтобы не извлекать корень)
        float distSq = dx*dx + dz*dz;

        // Порог отмены.
        // renderDistanceXZ + 2 (небольшой запас, чтобы не мерцало на границе)
        float cancelRadius = static_cast<float>(renderDistanceXZ + 2);
        float cancelDistSq = cancelRadius * cancelRadius;

        if (distSq > cancelDistSq) {
            // --- ЗАДАЧА УСТАРЕЛА ---

            // Очень важно: Удаляем флаг из pendingGeneration!
            // Иначе система будет думать, что чанк "генерируется" вечно,
            // и никогда не попытается загрузить его снова, если игрок вернется.
            {
                std::lock_guard<std::mutex> genLock(generationMutex);
                pendingGeneration.erase(task.chunkPos);
            }

            // Пропускаем тяжелую генерацию и берем следующую задачу
            continue;
        }
        // =================================================================================


        // 4. Если мы здесь, значит чанк всё еще нужен. Генерируем!
...
void chunkFinder(const ChunkMap& chunks) {
    while (running) {
        glm::ivec3 playerPosCopy;
        {
            std::lock_guard<std::mutex> lock(playerPosMutex);
            playerPosCopy = currentPlayerChunk;
        }

        // Проверка: Если очередь и так забита, не тратим время на поиск новых задач.
        // Даем воркеру разгрести то, что есть. Это гарантирует свежесть задач.
        size_t currentQueueSize = 0;
        {
             std::lock_guard<std::mutex> qLock(generationMutex);
             currentQueueSize = generationQueue.size();
        }

        // Если в очереди уже больше 50 чанков, ждем и пропускаем цикл.
        // 50 - это примерно на 100-200 мс работы воркера.
        ...
        // Используем структуру для внутренней сортировки
        struct Candidate {
            glm::ivec3 pos;
            float distSq;
        };
        ...
        for (int dx = -renderDistanceXZ; dx <= renderDistanceXZ; dx++) {
            for (int dz = -renderDistanceXZ; dz <= renderDistanceXZ; dz++) {
                for (int dy = -renderHeightY; dy <= renderHeightY; dy++) {
                    glm::ivec3 chunkPos = playerPosCopy + glm::ivec3(dx, dy, dz);

                    // Быстрая проверка "на глаз" перед тяжелой проверкой в map
                    // Если чанк уже генерируется - пропускаем
                    {
                         // Можно сделать "грязное чтение" без мьютекса для скорости,
                         // или с мьютексом, если pendingGeneration меняется часто.
                         // Для оптимизации лучше проверить это ПОСЛЕ сортировки, но ДО добавления.
                    }

                    if (chunks.tryGet(chunkPos) == nullptr) {
        ...
        // --- Этап 2: Сортировка ---
        // Мы хотим добавить в очередь САМЫЕ БЛИЗКИЕ из найденных.
        // Сортируем вектор: меньшая дистанция -> начало вектора.
        std::sort(candidates.begin(), candidates.end(),
            [](const Candidate& a, const Candidate& b) {
                return a.distSq < b.distSq;
            });

        // --- Этап 3: Добавление в очередь (с лимитом) ---
        if (!candidates.empty()) {
            std::lock_guard<std::mutex> genLock(generationMutex);

            // Добавляем только пока очередь не заполнится до лимита
            // Или пока не кончатся кандидаты
            int addedCount = 0;
            const int MAX_BATCH_ADD = 96; // Не добавляем больше 20 за раз

            for (const auto& cand : candidates) {
                // Если мы уже добавили достаточно задач на этот кадр - стоп.
                // Остальные добавим в следующем цикле, пересчитав их дистанцию заново!
                if (generationQueue.size() >= MAX_BATCH_ADD*2 || addedCount >= MAX_BATCH_ADD) break;

                if (pendingGeneration.count(cand.pos) == 0) {
                    pendingGeneration.insert(cand.pos);
                    // Push в priority_queue
                    generationQueue.push({ cand.pos, cand.distSq });
                    addedCount++;
                }
            }
        ...
        // --- Этап 3: Поиск чанков для ВЫГРУЗКИ ---
        std::vector<glm::ivec3> toUnload;
        // Мы не можем изменять `chunks` во время итерации, поэтому сначала собираем кандидатов
        auto chunkList = chunks.getSnapshot();
        for (const auto& pair : chunkList) {
            const glm::ivec3& chunkPos = pair.first;

            // ИСПРАВЛЕНИЕ: Проверяем расстояние по всем осям
            int dist_x = abs(chunkPos.x - playerPosCopy.x);
            int dist_z = abs(chunkPos.z - playerPosCopy.z);
            int dist_y = abs(chunkPos.y - playerPosCopy.y);

            // Вычисляем максимальное расстояние по горизонтали (расстояние Чебышева)
            int horizontal_dist = std::max(dist_x, dist_z);

            // Условие для выгрузки:
            // 1. Чанк слишком далеко по горизонтали.
            // ИЛИ
            // 2. Чанк слишком далеко по вертикали.
            // Добавляем буфер (+2), чтобы избежать постоянной загрузки/выгрузки на границе.
            if (horizontal_dist > renderDistanceXZ + 2 || dist_y > renderHeightY + 2) {
                toUnload.push_back(chunkPos);
            }
        }

        // Если есть что выгружать, добавляем в очередь
        if (!toUnload.empty()) {
            std::lock_guard<std::mutex> lock(unloadMutex);
            for(const auto& pos : toUnload) {
                // Дополнительная проверка: убедимся, что чанк все еще существует,
                // на случай если он был выгружен другим условием в промежутке.
